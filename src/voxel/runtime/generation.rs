...
fn should_attempt_saved_world_load(persistence_settings: &WorldPersistence) -> bool {
        ...
        || env_flag("VOXEL_FORCE_REGENERATE_WATER")
    {
        match persistence::delete_saved_world_at_path(&persistence_settings.path) {
            Ok(()) => info!("Water body regeneration requested; deleted saved world so terrain can regenerate"),
    ...
    persistence::saved_world_exists_at_path(&persistence_settings.path)
}

fn load_saved_world_for_runtime(persistence_settings: &WorldPersistence) -> Result<VoxelWorld, String> {
    if env_flag("DRUSNIEL_EDITOR_NATIVE_VIEWPORT") || persistence_settings.allow_terrain_fingerprint_mismatch {
    ...
    info!("Loading saved world from {}...", persistence_settings.path.display());
...
impl WorldStats {
    ...
    fn log_summary(&self, generation_time: std::time::Duration) {
        ...
        let skip_percent = if total_chunks > 0 { (skippable as f32 / total_chunks as f32) * 100.0 } else { 0.0 };
        info!("=== WORLD GENERATION SUMMARY ===");
        info!("Generation time: {:.2}s", generation_time.as_secs_f32());
        info!("--- Chunk Uniformity (mesh optimization) ---");
        if total_chunks > 0 {
            info!("  Empty chunks (all air): {} ({:.1}% of total)", self.empty_chunks, (self.empty_chunks as f32 / total_chunks as f32) * 100.0);
            info!("  Solid chunks (no internal surfaces): {} ({:.1}% of total)", self.solid_chunks, (self.solid_chunks as f32 / total_chunks as f32) * 100.0);
            info!("  Mixed chunks (need full meshing): {} ({:.1}% of total)", self.mixed_chunks, (self.mixed_chunks as f32 / total_chunks as f32) * 100.0);
        }
        info!("  Skippable chunks: {}/{} ({:.1}%)", skippable, total_chunks, skip_percent);
...
pub(crate) fn start_voxel_world_after_overlay_frame(
    ...
    if should_attempt_saved_world_load(&persistence_settings) {
        ...
        let task = AsyncComputeTaskPool::get().spawn(async move { load_saved_world_for_runtime(&persistence_settings) });
...
pub(crate) fn chunk_terrain_source_for_config(config: &TerrainSourceConfig, biome_table: BiomeTable) -> ChunkTerrainSource {
    match config.mode {
        TerrainSourceMode::Legacy => ChunkTerrainSource::Legacy(Arc::new(TerrainGenerator::with_biome_table(ValueNoise::default(), biome_table))),
        TerrainSourceMode::GpuWorldSource | TerrainSourceMode::CpuWorldSourceReference => {
            ChunkTerrainSource::WorldSource(Arc::new(ProceduralWorldSourceTerrainBridge::load_or_default()), config.mode)
        }
    }
}
...
pub(crate) fn begin_world_generation(
    ...
    info!("Queued {} async chunk generation tasks for batched startup spawning", total_chunks);
}
...
pub(crate) fn poll_world_load_task(
    ...
    for (entity, mut task) in tasks.iter_mut() {
        let Some(result) = block_on(poll_once(&mut task.task)) else { continue; };
        commands.entity(entity).despawn();
        match result {
            Ok(loaded_world) => {
                if loaded_world.world_size_chunks() != world.world_size_chunks() {
                    warn!("Saved world size {:?} does not match configured world size {:?}; regenerating", loaded_world.world_size_chunks(), world.world_size_chunks());
                ...
                if loaded_chunks != expected_chunks {
                    warn!("Saved world contains {}/{} chunks; regenerating incomplete save", loaded_chunks, expected_chunks);
                    request_world_generation(&mut gen_state, &mut pending_generation);
                    continue;
                }
                *world = loaded_world;
            ...
            Err(err) => {
                warn!("Failed to load saved world: {}. Generating new world...", err);
                request_world_generation(&mut gen_state, &mut pending_generation);
            }
        }
    }
...
fn request_world_generation(gen_state: &mut ChunkGenerationState, pending_generation: &mut PendingWorldGeneration) {
    gen_state.total_chunks = 0;
    gen_state.chunks_completed = 0;
    gen_state.is_complete = false;
    gen_state.loading_from_disk = false;
...
pub(crate) fn spawn_queued_chunk_generation_tasks(mut commands: Commands, mut generation_queue: ResMut<WorldGenerationQueue>) {
    let Some((chunk_positions, generator, complete)) = generation_queue.take_next_batch(WORLD_GENERATION_TASK_SPAWN_BATCH) else { return; };
    ...
    if complete {
        ...
        debug!("Spawned {} queued chunk generation tasks this frame; {} remaining", spawned_count, generation_queue.remaining());
    }
}
...
pub(crate) fn generate_chunk_async(chunk_pos: IVec3, source: &ChunkTerrainSource) -> (Chunk, ChunkStats) {
    match source {
...
fn generate_world_source_chunk_async(chunk_pos: IVec3, bridge: &ProceduralWorldSourceTerrainBridge) -> (Chunk, ChunkStats) {
...
fn generate_legacy_chunk_async(chunk_pos: IVec3, generator: &TerrainGenerator) -> (Chunk, ChunkStats) {
    ...
    if DEBUG_FLAT_WORLD {
        ...
        fill_chunk_voxels(&mut voxels, &columns, chunk_world_x, chunk_world_y, chunk_world_z, generator);
        let trees = precompute_overlapping_trees(chunk_world_x, chunk_world_z, &columns, generator);
        paint_tree_leaves(&mut voxels, &columns, &trees, chunk_world_x, chunk_world_y, chunk_world_z);
    }
    let stats = collect_chunk_stats(&voxels);
    let chunk = Chunk::with_voxels(chunk_pos, voxels);
    (chunk, stats)
...
fn precompute_terrain_columns(chunk_world_x: i32, chunk_world_z: i32, generator: &TerrainGenerator) -> [TerrainColumn; CHUNK_SIZE * CHUNK_SIZE] {
    std::array::from_fn(|index| {
        let x = index % CHUNK_SIZE;
        let z = index / CHUNK_SIZE;
        let world_x = chunk_world_x + x as i32;
        let world_z = chunk_world_z + z as i32;
        let (terrain_height, water) = generator.get_height_and_water_generation_metadata(world_x, world_z);
        let biome = generator.get_biome(world_x, world_z);
        let tree = tree_at(generator, world_x, world_z, terrain_height);
        TerrainColumn { terrain_height, biome, water, tree }
    })
}

fn precompute_overlapping_trees(chunk_world_x: i32, chunk_world_z: i32, columns: &[TerrainColumn; CHUNK_SIZE * CHUNK_SIZE], generator: &TerrainGenerator) -> Vec<GeneratedTree> {
    ...
    for world_z in min_z..=max_z {
        for world_x in min_x..=max_x {
            let terrain_height = tree_scan_terrain_height(chunk_world_x, chunk_world_z, columns, generator, world_x, world_z);
...
fn tree_scan_terrain_height(chunk_world_x: i32, chunk_world_z: i32, columns: &[TerrainColumn; CHUNK_SIZE * CHUNK_SIZE], generator: &TerrainGenerator, world_x: i32, world_z: i32) -> i32 {
    ...
    if (0..CHUNK_SIZE_I32).contains(&local_x) && (0..CHUNK_SIZE_I32).contains(&local_z) {
        ...
        generator.get_height_and_water_generation_metadata(world_x, world_z).0
    }
}

fn tree_at(generator: &TerrainGenerator, world_x: i32, world_z: i32, terrain_height: i32) -> Option<GeneratedTree> {
    ...
    Some(GeneratedTree { world_x, world_z, trunk_top, leaf_center_y: trunk_top - 1 })
}

fn fill_chunk_voxels(voxels: &mut [VoxelType; CHUNK_VOLUME], columns: &[TerrainColumn; CHUNK_SIZE * CHUNK_SIZE], chunk_world_x: i32, chunk_world_y: i32, chunk_world_z: i32, generator: &TerrainGenerator) {
    for x in 0..CHUNK_SIZE {
        for z in 0..CHUNK_SIZE {
            let world_x = chunk_world_x + x as i32;
            let world_z = chunk_world_z + z as i32;
            ...
            for y in 0..CHUNK_SIZE {
                let world_y = chunk_world_y + y as i32;
                voxels[Chunk::index(x, y, z)] = voxel_from_column(generator, column, world_x, world_y, world_z);
            }
        }
    }
}

fn voxel_from_column(generator: &TerrainGenerator, column: TerrainColumn, world_x: i32, world_y: i32, world_z: i32) -> VoxelType {
    if world_y <= BEDROCK_DEPTH {
        return VoxelType::Bedrock;
    }
    if generator.is_cave(world_x, world_y, world_z, column.terrain_height) {
        return if generator.is_cave_aquifer(world_x, world_y, world_z) { VoxelType::Water } else { VoxelType::Air };
    }
    if column.tree.is_some_and(|tree| world_y >= column.terrain_height + 1 && world_y < tree.trunk_top) {
        return VoxelType::Wood;
    }
    if world_y > column.terrain_height {
        return if column.water.is_surface_water() && world_y <= column.water.surface_y { VoxelType::Water } else { VoxelType::Air };
    }
...
fn paint_tree_leaves(voxels: &mut [VoxelType; CHUNK_VOLUME], columns: &[TerrainColumn; CHUNK_SIZE * CHUNK_SIZE], trees: &[GeneratedTree], chunk_world_x: i32, chunk_world_y: i32, chunk_world_z: i32) {
    ...
    for tree in trees {
        ...
        for world_z in min_z..=max_z {
            for world_x in min_x..=max_x {
                ...
                if xz_dist_sq >= leaf_radius_sq { continue; }
                for world_y in chunk_world_y..=chunk_max_y {
                    if world_y <= column.terrain_height { continue; }
                    let dy = world_y - tree.leaf_center_y;
                    let dist_sq = xz_dist_sq + (dy * dy) as f32 * 1.5;
                    if dist_sq >= leaf_radius_sq { continue; }
                    if dx == 0 && dz == 0 && world_y < tree.trunk_top { continue; }
...
pub(crate) fn poll_chunk_generation_tasks(
    ...
    for (entity, mut task) in tasks.iter_mut() {
        if let Some(result) = block_on(poll_once(&mut task.task)) {
            ...
            if stats.dungeon_wall > 0 || stats.dungeon_floor > 0 {
                let chunk_world = IVec3::new(chunk_pos.x * CHUNK_SIZE_I32, chunk_pos.y * CHUNK_SIZE_I32, chunk_pos.z * CHUNK_SIZE_I32);
                debug!("Chunk {:?} (world {:?}): {} dungeon walls, {} floors", chunk_pos, chunk_world, stats.dungeon_wall, stats.dungeon_floor);
            }
    ...
    if completed_count > 0 {
        ...
        let prev_progress_pct = ((gen_state.chunks_completed - completed_count) as f32 / gen_state.total_chunks as f32 * 100.0) as u32;
        if progress_pct / 10 > prev_progress_pct / 10 {
            info!("World generation: {}% ({}/{}) chunks", progress_pct, gen_state.chunks_completed, gen_state.total_chunks);
        }
    }
    if gen_state.chunks_completed >= gen_state.total_chunks {
        gen_state.is_complete = true;
        info!("Terrain mesh build COMPLETE: {} chunks meshed", gen_state.chunks_completed);
...
mod tests {
    ...
    fn terrain_source_config_selects_legacy_generation() {
        let source = chunk_terrain_source_for_config(&TerrainSourceConfig { mode: TerrainSourceMode::Legacy }, BiomeTable::default());
    ...
    fn terrain_source_config_selects_gpu_world_source_generation() {
        let source = chunk_terrain_source_for_config(&TerrainSourceConfig { mode: TerrainSourceMode::GpuWorldSource }, BiomeTable::default());
    ...
    fn terrain_source_config_selects_explicit_cpu_reference_generation() {
        let source = chunk_terrain_source_for_config(&TerrainSourceConfig { mode: TerrainSourceMode::CpuWorldSourceReference }, BiomeTable::default());
