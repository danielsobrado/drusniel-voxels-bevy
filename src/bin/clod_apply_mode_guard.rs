...
fn main() {
    ...
    let cfg_path = args
        .get(2)
        .map(String::as_str)
        .unwrap_or("assets/config/clod_apply_mode_guard.toml");
    ...
    if requests.is_empty() {
        errors.push("no mutation requests found".to_string());
    }
    ...
    for req in &requests {
        ...
        if cfg.require_authoritative_acceptance {
            match hook.map(|r| get(r, "decision")) {
                ...
                other => errors.push(format!(
                    "request {id} not accepted by authoritative hook: {:?}",
                    other
                )),
            }
        }
        if !cfg.allow_pending_authoritative_apply {
            if let Some(h) = hook {
                if get(h, "decision") == "hook_unavailable" {
                    errors.push(format!("hook unavailable for {id}"));
                }
            }
        }
    ...
    if cfg.require_rebuild_after_apply
        && !rebuilds
            .iter()
            .any(|r| get(r, "phase") == "published" || get(r, "stage") == "published")
    {
    ...
    lines
        ...
        .map(|line| {
            headers
                .iter()
                .cloned()
                .zip(line.split(',').map(|s| s.trim().to_string()))
                .collect()
        })
        .collect()
}
fn by_request_id(rows: &[Row]) -> HashMap<&str, &Row> {
    rows.iter()
        .filter_map(|r| r.get("request_id").map(|id| (id.as_str(), r)))
        .collect()
}
...
fn parse_config(toml: &str) -> Config {
    let mut c = Config {
        ...
        max_apply_to_rebuild_latency_frames: 180,
        max_apply_to_collider_latency_frames: 180,
        ..Default::default()
    };
...
fn fail(msg: &str) -> ! {
    eprintln!("[CLOD APPLY MODE GUARD] FAILED\n{msg}");
    process::exit(1);
}
