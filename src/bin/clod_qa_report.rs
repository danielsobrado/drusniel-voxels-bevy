...
const KNOWN_ARTIFACTS: &[(&str, &str)] = &[
    ...
    ("edit_mutation_requests", "clod-edit-mutation-requests.csv"),
    ("edit_mutation_sink", "clod-edit-mutation-sink.csv"),
    (
        "edit_authoritative_hook",
    ...
    ("weld", "clod-weld.csv"),
    ("collider_refresh", "clod-collider-refresh.csv"),
];
...
fn write_markdown(args: &Args, summaries: &[CsvSummary]) -> io::Result<()> {
    ...
    writeln!(out)?;
    writeln!(out, "```bash")?;
    writeln!(out, "scripts/run-clod-complete-qa.sh")?;
    writeln!(out, "```")?;
    Ok(())
}
