...
fn main() {
    ...
    for row in &rows {
        ...
        if cfg.require_dry_run_only && decision != "dry_run" {
            errors.push(format!(
                "non-dry-run collider decision for {}: {}",
                row.get("request_id"),
                decision
            ));
        }
        if decision == "refreshed" && !cfg.allow_refreshed {
            errors.push(format!(
                "refreshed row not allowed: {}",
                row.get("request_id")
            ));
        }
        if decision == "stale" && !cfg.allow_stale {
            errors.push(format!("stale collider row: {}", row.get("request_id")));
        }
        if decision == "timeout" && !cfg.allow_timeout {
            errors.push(format!(
                "collider refresh timeout: {}",
                row.get("request_id")
            ));
        }
        if decision == "pending_authoritative_apply" && !cfg.allow_missing_refresh_when_dry_run {
            errors.push(format!(
                "pending refresh not allowed: {}",
                row.get("request_id")
            ));
        }
        let dirty = row.get("dirty_lod0_pages").parse::<i64>().unwrap_or(0);
        if dirty > cfg.max_dirty_lod0_pages {
            errors.push(format!(
                "dirty_lod0_pages too high for {}: {}",
                row.get("request_id"),
                dirty
            ));
        }
        let latency = row
            .get("refresh_latency_frames")
            .parse::<i64>()
            .unwrap_or(-1);
        if latency >= 0 && latency > cfg.max_refresh_latency_frames {
            errors.push(format!(
                "refresh latency too high for {}: {}",
                row.get("request_id"),
                latency
            ));
        }
        let stale = row.get("stale_collider_frames").parse::<i64>().unwrap_or(0);
        if stale > cfg.max_stale_collider_frames {
            errors.push(format!(
                "stale collider frames for {}: {}",
                row.get("request_id"),
                stale
            ));
        }
...
impl Row {
    fn get(&self, name: &str) -> &str {
        self.headers
            ...
            .and_then(|i| self.fields.get(i).map(|s| s.as_str()))
            .unwrap_or("")
    }
}
fn parse_rows(csv: &str) -> Vec<Row> {
    ...
    lines
        ...
        .map(|l| Row {
            ...
            fields: l.split(',').map(|s| s.trim().to_string()).collect(),
        })
        .collect()
}
fn parse_config(toml: &str) -> Config {
    let mut c = Config {
        ...
        allow_missing_refresh_when_dry_run: true,
        max_refresh_latency_frames: 180,
...
fn fail(msg: &str) -> ! {
    eprintln!("[CLOD COLLIDER REFRESH GUARD] FAILED\n{msg}");
    process::exit(1);
}
