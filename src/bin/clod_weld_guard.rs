    ...
    for (line_no, line) in lines.enumerate() {
        ...
        if dup_groups != 0 {
            failures.push(format!(
                "{label}: duplicate_position_groups {dup_groups} != 0"
            ));
        }
        if dup_vertices != 0 {
            failures.push(format!("{label}: duplicate_vertices {dup_vertices} != 0"));
        }
        if normal > 1.0e-4 {
            failures.push(format!("{label}: max_normal_delta {normal:.8} > 0.0001"));
        }
        if material > 1.0e-4 {
            failures.push(format!(
                "{label}: max_material_delta {material:.8} > 0.0001"
            ));
        }
        if paint > 1.0e-4 {
            failures.push(format!("{label}: max_paint_delta {paint:.8} > 0.0001"));
        }
    }
    if rows == 0 {
        failures.push("no weld diagnostic rows found".to_string());
    }
