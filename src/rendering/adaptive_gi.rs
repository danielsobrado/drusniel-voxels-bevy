...
fn update_shadow_settings(settings: Res<AdaptiveGISettings>, state: Res<AdaptiveGIState>) {
...
fn handle_quality_hotkeys(
    ...
    if keyboard.pressed(KeyCode::AltLeft) || keyboard.pressed(KeyCode::AltRight) {
        ...
        if let Some(quality) = new_quality {
            ...
            info!(
                "Adaptive GI quality set to: {:?} (perf factor: {:.0}%)",
                quality,
                quality.performance_factor() * 100.0
            );
        }
        ...
        if keyboard.just_pressed(KeyCode::KeyC) {
            ...
            info!(
                "Contact shadow debug: {}",
                if settings.debug_contact_shadows {
                    "ON"
                } else {
                    "OFF"
                }
    ...
    pub fn draw_adaptive_gi_debug_ui(
        ...
        ui.horizontal(|ui| {
            for quality in AdaptiveGIQuality::all() {
                if ui
                    .selectable_label(state.current_quality == *quality, quality.display_name())
                    .clicked()
                {
        ...
        ui.collapsing("Stochastic Probes", |ui| {
            ui.checkbox(
                &mut settings.stochastic_probe_selection,
                "Enable Stochastic Selection",
            );
            if settings.stochastic_probe_selection {
                ui.add(
                    bevy_egui::egui::Slider::new(&mut settings.probe_selection_count, 2..=8)
                        .text("Probe Count (1-from-N)"),
                );
                ui.checkbox(
                    &mut settings.blue_noise_probe_selection,
        ...
        ui.collapsing("SDF Shadows", |ui| {
            ...
            if settings.sdf_shadows_enabled {
                ui.add(
                    bevy_egui::egui::Slider::new(&mut settings.sdf_shadow_steps, 8..=64)
                        .text("Ray Steps"),
                );
                ui.add(
                    bevy_egui::egui::Slider::new(&mut settings.sdf_shadow_softness, 1.0..=16.0)
                        .text("Softness"),
                );
                ui.add(
                    bevy_egui::egui::Slider::new(
                        &mut settings.sdf_shadow_max_distance,
                        16.0..=128.0,
                    )
                    .text("Max Distance"),
                );
        ...
        ui.collapsing("Contact Shadows", |ui| {
            ui.checkbox(
                &mut settings.contact_shadows_enabled,
                "Enable Contact Shadows",
            );
            if settings.contact_shadows_enabled {
                ui.add(
                    bevy_egui::egui::Slider::new(&mut settings.contact_shadow_steps, 4..=16)
                        .text("Ray Steps"),
                );
                ui.add(
                    bevy_egui::egui::Slider::new(&mut settings.contact_shadow_length, 0.5..=5.0)
                        .text("Ray Length"),
                );
                ui.add(
                    bevy_egui::egui::Slider::new(
                        &mut settings.contact_shadow_fade_distance,
                        20.0..=100.0,
                    )
                    .text("Fade Distance"),
                );
        ...
        ui.collapsing("Vegetation", |ui| {
            ...
            ui.add(
                bevy_egui::egui::Slider::new(&mut settings.grass_ao_strength, 0.0..=1.0)
                    .text("AO Strength"),
            );
            ui.add(
                bevy_egui::egui::Slider::new(&mut settings.grass_density, 0.0..=1.0)
                    .text("Density Factor"),
            );
        });
        ...
        ui.collapsing("Debug", |ui| {
            ui.checkbox(
                &mut settings.debug_probe_selection,
                "Visualize Probe Selection",
            );
            ui.checkbox(
                &mut settings.debug_contact_shadows,
                "Visualize Contact Shadows",
            );
            ui.label(format!("Frame: {}", state.frame_index));
            ui.label(format!("Quality: {:?}", state.current_quality));
            ui.label(format!(
                "Perf Factor: {:.0}%",
                state.current_quality.performance_factor() * 100.0
            ));
        });
    }
}
...
mod tests {
    ...
    fn performance_factors_are_reasonable() {
        ...
        assert!(
            AdaptiveGIQuality::Low.performance_factor()
