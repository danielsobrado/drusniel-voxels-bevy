...
scripts/export-clod-edit-dispatch.sh "$EVENTS_CSV" "$DISPATCH_CSV"

step "exporting dry-run edit audit: $DRY_RUN_CSV"
scripts/export-clod-edit-dry-run.sh "$DISPATCH_CSV" "$DRY_RUN_CSV"

step "exporting mutation requests: $MUTATION_REQUESTS_CSV"
scripts/export-clod-edit-mutation-requests.sh "$DRY_RUN_CSV" "$MUTATION_REQUESTS_CSV"

step "exporting mutation sink audit: $MUTATION_SINK_CSV"
scripts/export-clod-edit-mutation-sink.sh "$MUTATION_REQUESTS_CSV" "$MUTATION_SINK_CSV"

step "exporting authoritative hook audit: $AUTHORITATIVE_HOOK_CSV"
scripts/export-clod-edit-authoritative-hook.sh "$MUTATION_REQUESTS_CSV" "$AUTHORITATIVE_HOOK_CSV"

step "exporting collider refresh audit: $COLLIDER_REFRESH_CSV"
scripts/export-clod-collider-refresh.sh "$AUTHORITATIVE_HOOK_CSV" "$COLLIDER_REFRESH_CSV"
...
scripts/guard-clod-edit-dry-run.sh "$DRY_RUN_CSV"

step "guarding mutation requests"
scripts/guard-clod-edit-mutation-requests.sh "$MUTATION_REQUESTS_CSV"

step "guarding mutation sink audit"
scripts/guard-clod-edit-mutation-sink.sh "$MUTATION_SINK_CSV"

step "guarding authoritative hook audit"
scripts/guard-clod-edit-authoritative-hook.sh "$AUTHORITATIVE_HOOK_CSV"

step "guarding collider refresh audit"
scripts/guard-clod-collider-refresh.sh "$COLLIDER_REFRESH_CSV"
