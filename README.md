...
* **Experimental NAADF Backend**: Added a default-off derived voxel ray cache for GI/preview experiments. It is opt-in through the `naadf` Cargo feature and [assets/config/naadf.yaml](assets/config/naadf.yaml), keeps the current renderer as the shipping default, and preserves integrated-GPU fallback unless explicitly overridden. See [docs/rendering/naadf.md](docs/rendering/naadf.md), [docs/rendering/naadf-debugging.md](docs/rendering/naadf-debugging.md), [docs/rendering/naadf-benchmarks.md](docs/rendering/naadf-benchmarks.md), and [docs/rendering/naadf-completion-jira-plan.md](docs/rendering/naadf-completion-jira-plan.md).
  ...
  * **GPU chunk-bound pass**: `build_chunk_bounds.wgsl` computes safe contiguous loaded-empty chunk skips into chunk record word 6, so GPU-built records are no longer limited to CPU-uploaded chunk skip data.
  ...
  * **Preview/filter compute bodies**: First-hit preview, temporal accumulation, spatial resampling, and preview composite shaders now have dispatchable compute entry points instead of helper-only bodies.
  * **Render graph preview node**: NAADF preview mode now registers a Core3d render graph view node that loads NAADF shaders, creates compute/render pipelines, extracts preview activation state into the render world, dispatches GPU build stages, renders a first-hit preview texture through a GPU chunk lookup table, runs an edge-aware spatial filter from first-hit color/depth/normal textures, accumulates against per-view temporal history, and composites it over the current view.
  ...
  * **Persistent preview history**: The preview graph keeps per-view ping-pong `rgba16float` history textures in the render world and resets them when preview history generation or viewport size changes.
  * **HUD/backend fallback polish**: NAADF runtime selection now reports clearer fallback reasons when the feature is not compiled or the runtime needs to be restarted with NAADF enabled.
  ...
  Remaining NAADF gaps are still documented rather than hidden: GPU chunk-bound propagation is a safe contiguous-neighbor pass rather than the full upstream queue solver, and upstream parity items such as atmosphere integration, dynamic entity voxel volumes, advanced GI, SVGF-quality denoise, and full final compositing are future work.
