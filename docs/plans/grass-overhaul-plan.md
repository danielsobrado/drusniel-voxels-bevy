...
Document status: older execution plan for replacing dense object-style grass with terrain-derived patch grass in the CLOD PoC first, then the Rust/Bevy runtime. The current CLOD-POC WebGPU ring direction is tracked in [`clod-poc-grass-port-plan.md`](clod-poc-grass-port-plan.md); keep this plan as background for terrain qualification, edit behavior, and Rust follow-up.
...
For CLOD-POC specifically, prefer the Fable5 `GroundRing` model described in [`clod-poc-grass-port-plan.md`](clod-poc-grass-port-plan.md): camera-following WebGPU ring, deterministic world cells, GPU cull/compact, indirect draws, and grass LOD bands. The Eclipse material remains useful for terrain eligibility and distance-simplification ideas, not as the primary ownership model.
...
- The authoritative CLOD-POC grass ring plan is `docs/plans/clod-poc-grass-port-plan.md`.
