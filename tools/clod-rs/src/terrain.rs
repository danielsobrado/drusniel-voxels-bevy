//! Synthetic terrain stand-in for the engine's Surface Nets mesher. Port of terrain.ts.
...
pub fn density(x: f32, y: f32, z: f32) -> f32 {
    ...
    let dx = x - 128.0;
    let dz = z - 128.0;
    let bulge = 6.0 * (-(dx * dx + dz * dz) / 900.0).exp() * (-((y - 30.0) * (y - 30.0)) / 120.0).exp();
    base + bulge
}
...
pub fn mesh_chunk(cx: i32, cz: i32, cfg: &ClodPagesConfig, world: WorldBounds) -> PageMesh {
