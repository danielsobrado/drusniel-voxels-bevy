//! Builder validation — errors, never warnings. Port of validate.ts (§3.3 / §11.7).

use crate::types::{ClodBuildError, PageFootprint, PageMesh, DEFAULT_TOLERANCES};
...
/// After internal welding this IS the page's outer border (plan §3.1) — detect by topology,
/// not by footprint-plane position (Surface Nets borders are non-planar).
...
// Surface-nets borders hug the perimeter within ~1 cell; an unwelded internal seam shows
// up as open edges far from the perimeter — that's what we catch.
...
pub fn assert_no_internal_borders(mesh: &PageMesh, fp: &PageFootprint) -> Result<(), ClodBuildError> {
    let flags = open_boundary_vertex_flags(mesh);
    for (i, &open) in flags.iter().enumerate() {
        if !open {
            continue;
        }
        ...
        if d > PERIMETER_BAND {
            return Err(ClodBuildError::InternalBorderNotWelded(format!(
                "open-boundary vertex ({:.2},{:.2},{:.2}) is {:.2} cells from the footprint perimeter",
                p[0], p[1], p[2], d
            )));
        }
    }
