...
export function evaluateWaterAcceptance(input: WaterAcceptanceInput): string[] {
  ...
  const highQualityMaterialActive = finiteValue(input.counters, "water_high_quality_material_active");
  ...
  if (highQualityMaterialActive !== 1) {
    failures.push(`water_high_quality_material_active=${highQualityMaterialActive} must equal 1`);
  }
