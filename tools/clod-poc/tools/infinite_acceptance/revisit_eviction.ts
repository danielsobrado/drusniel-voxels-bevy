...
function category(
  ...
  options: { required: boolean; mustEvict?: boolean; filter?: (key: string) => boolean },
  ...
  const mustEvict = options.mustEvict ?? true;
  const passed = targetKeys.length > 0 && (!mustEvict || remainingKeys.length === 0);
  const failure = targetKeys.length === 0
    ? `${name}: no target keys were resident at route A`
    : mustEvict && remainingKeys.length > 0
...
export function evaluateRevisitEviction(
  ...
  const heightfield = category("heightfield tiles", atRouteA.heightfieldResidentKeys, beforeReturn.heightfieldResidentKeys, { required: true, mustEvict: false });
  ...
  const waterHydrology = category("water/hydrology", atRouteA.waterHydrologyKeys, beforeReturn.waterHydrologyKeys, { required: false, mustEvict: false });
