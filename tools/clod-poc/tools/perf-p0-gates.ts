...
function gateP0DirtyAtlasExerciseCompleted(byName: ReadonlyMap<string, P0PerfGateCaseLike>): P0PerfGateResult {
  ...
  const bestBumped = completed.reduce((max, perfCase) => Math.max(max, metric(perfCase, "p0DirtyAtlasExercise.bumpedTiles") ?? 0), 0);
  return gate(
    "p0-dirty-atlas-exercise-completed",
    enabledCases.length > 0 && completed.length > 0 && skipped.length === 0,
    enabledCases.length > 0 && completed.length > 0 && skipped.length === 0
      ? `dirty atlas exercise completed cases=${completed.length}/${enabledCases.length} bestBumpedTiles=${formatMetric(bestBumped)}`
