...
export function evaluateWaterFoamRuntimeContract(
  ...
  requireEqual(
    failures,
    "shore-distance weight",
    diagnostics.shoreDistanceWeight,
    WATER_FOAM_SHORE_DISTANCE_WEIGHT,
  );
  requireEqual(
    failures,
    "river-shore attenuation",
    diagnostics.riverShoreAttenuation,
    WATER_FOAM_RIVER_SHORE_ATTENUATION,
  );
  requireEqual(
    failures,
    "shade coverage floor",
    diagnostics.shadeCoverageFloor,
    WATER_FOAM_SHADE_COVERAGE_FLOOR,
  );
  requireEqual(
    failures,
    "rapid eligibility",
    diagnostics.rapidEligibility,
    "speed-times-drop-times-river",
  );
...
function requireMin(failures: string[], label: string, actual: number, min: number): void {
  if (!Number.isFinite(actual) || actual < min) {
    failures.push(`${label} ${String(actual)} is below ${String(min)}`);
  }
}
