...
const BENCHMARK_SOURCE = path.resolve(process.cwd(), "tools", "benchmark-gpu-clod-resident.ts");
...
function spawnChild(label, command, args, pipeOutput = false) {
  ...
  child.on("error", (error) => console.error(`[gpu-clod-resident:${label}] failed to start: ${error.message}`));
...
async function ensureServer() {
  ...
  if (alreadyReady && reuseExistingServer()) {
    console.log(`[gpu-clod-resident] reusing existing Vite at ${baseUrl}`);
    return null;
  }
  if (alreadyReady) {
    throw new Error(
  ...
  console.log(`[gpu-clod-resident] starting Vite at ${baseUrl}`);
