...
export function evaluateWaterFoamQualityParity(
  ...
  const measurements = {
    ...
    rapidLargestComponentDelta: absoluteDelta(
      low.rapid.largestComponentFraction,
      high.rapid.largestComponentFraction,
    ),
    rapidStripeDelta: absoluteDelta(low.rapid.stripeAnisotropy, high.rapid.stripeAnisotropy),
    rapidIsolatedDelta: absoluteDelta(
      low.rapid.isolatedActiveFraction,
      high.rapid.isolatedActiveFraction,
    ),
    ...
    lakeShoreCoverageDelta: absoluteDelta(low.lakeShore.meanCoverage, high.lakeShore.meanCoverage),
    lightingMeanDelta: absoluteDelta(
      low.rapidLighting.meanLuminance,
      high.rapidLighting.meanLuminance,
    ),
    lightingVariationRatio: ratio(
      low.rapidLighting.standardDeviation,
      high.rapidLighting.standardDeviation,
    ),
    temporalDeltaRatio: ratio(
      low.rapidTemporal.meanAbsoluteDelta,
      high.rapidTemporal.meanAbsoluteDelta,
    ),
    temporalIouDelta: absoluteDelta(low.rapidTemporal.binaryIou, high.rapidTemporal.binaryIou),
  };
  const failures: string[] = [];

  requireRange(
    failures,
    "rapid active ratio low/high",
  ...
  requireRange(
    failures,
    "rapid mean coverage ratio low/high",
  ...
  requireMin(
    failures,
    "lit-foam luminance-variation ratio low/high",
    measurements.lightingVariationRatio,
    limits.lightingVariationRatioMin,
  );
  requireRange(
    failures,
    "rapid temporal-delta ratio low/high",
...
function absoluteDelta(a: number, b: number): number {
...
function requireMin(failures: string[], label: string, value: number, min: number): void {
  if (!Number.isFinite(value) || value < min) {
    failures.push(`${label} ${format(value)} is below ${min.toFixed(4)}`);
  }
}

function requireMax(failures: string[], label: string, value: number, max: number): void {
  if (!Number.isFinite(value) || value > max) {
    failures.push(`${label} ${format(value)} exceeds ${max.toFixed(4)}`);
  }
}
