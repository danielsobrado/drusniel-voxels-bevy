// Reproduce the user's in-canopy camera pose (X256 Z263 H~34, among the trunks) and
...
// customProps=1 keeps the oceanRim world but enables the setPose automation hook.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await page.waitForTimeout(Number(process.env.SETTLE_MS ?? 30000));
    ...
    await page.waitForTimeout(2500); // let streaming/selection settle at the new pose
...
function analyse(frames: Frame[], outDir: string): void {
  ...
  // Analyse the whole left 82% of frame (skip GUI chrome on the right).
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let t = 0; t < luma.length; t++) { const v = luma[t][i]; if (v < lo) lo = v; if (v > hi) hi = v; if (t > 0 && Math.abs(luma[t][i] - luma[t-1][i]) > 60) pairChurn[t-1]++; }
  ...
  for (let i = 0; i < n; i++) { let lo = Infinity, hi = -Infinity; for (let t = 0; t < luma.length; t++) { const v = luma[t][i]; if (v < lo) lo = v; if (v > hi) hi = v; } const r = Math.min(255, (hi - lo) * 3); vis[i*3] = r; vis[i*3+1] = 0; vis[i*3+2] = 0; }
