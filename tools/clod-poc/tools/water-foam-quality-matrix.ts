...
function main(): void {
  ...
  for (const quality of WATER_FOAM_ACCEPTANCE_QUALITIES) {
    ...
    if (!existsSync(reportPath)) throw new Error(`foam acceptance ${quality} did not write ${reportPath}`);
    ...
    if (!canonicalPoses) {
      ...
      try {
        ...
        poseParity = false;
        poseFailures.push(error instanceof Error ? error.message : String(error));
      }
    }

    const visualFailures = Array.isArray(parsed.acceptance?.failures)
      ? parsed.acceptance.failures.filter((entry): entry is string => typeof entry === "string")
      : [];
    const failures = [...visualFailures, ...poseFailures];
  ...
  const passed = reports.every((report) => report.passed) && qualityParity.passed;
  const matrixReport = {
    schemaVersion: 1,
    seed,
    world,
