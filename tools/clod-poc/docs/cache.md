...
PoC-scoped persistent cache for generated CLOD page artifacts, terrain summary artifacts, and far-view helper schemas. This validates cache keys, serialization, invalidation, compression, and lazy streaming in TypeScript/Three.js before any Bevy/Rust port.

**Phase 9 status:** persistent reload coverage is complete for `clod-page-node` and `terrain-summary` artifacts. `clod-page-tree` is written as schema proof, but not yet used to accelerate tree reconstruction. Far-shell, shadow-proxy, and canopy summary producers are still schema-only.

**This is not production-ready.** It is a design validation step.

## What is cached

| Artifact kind | Contents | Persistence |
|---------------|----------|-------------|
| `clod-page-node` | Positions, normals, paint slots, material weights, indices, error/bounds metadata | IndexedDB via main-thread page broker |
| `clod-page-tree` | Page tree node ids, levels, child relationships | IndexedDB write only; read path not wired yet |
| `terrain-summary` | Coarse height/normal/coverage grids for far shell | IndexedDB from main thread |
| `far-shell-summary` | Schema supported; producer not wired yet | Not produced yet |
| `shadow-proxy-summary` | Schema supported; producer not wired yet | Not produced yet |
| `canopy-summary` | Schema supported; producer not wired yet | Not produced yet |
...
- Dig edit revision
- Hydrology carved bed full SHA-256 over raw `Float32Array` bytes when hydrology terrain is active
...
4. **Manifest** — In-memory LRU eviction tracker per cache service instance. Not persisted to IndexedDB. Services hydrate manifest entries by scanning artifact keys up to 256 entries.
5. **Page persistence** — CLOD worker never opens IndexedDB directly. It uses RPC to the main-thread broker database `drusniel-clod-poc-cache-pages-v2`.
6. **Terrain-summary persistence** — Main thread owns its own IndexedDB database `drusniel-clod-poc-cache-summary-v2`.
...
- **Clear persistent** — clears main memory cache, main terrain-summary IndexedDB, and worker page-node IndexedDB via worker message

Or delete these IndexedDB databases in browser devtools:

- `drusniel-clod-poc-cache-summary-v2`
- `drusniel-clod-poc-cache-pages-v2`
...
5. Check main section: terrain-summary hits should increase on reload after the first summary write
6. Compare console `[cache]` logs for hit rate, bytes read, decode ms, and `build avoided` ms
...
- `clod-page-tree` cache is written but not used to accelerate tree reconstruction yet
- Far-shell, shadow-proxy, and canopy summary artifact schemas exist, but producers are not wired yet
