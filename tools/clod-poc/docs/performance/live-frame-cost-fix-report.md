...
**30-second discriminator (do this first):** in the trees panel enable `debug color by LOD`. Debug colors are near=green, mid=orange, far=blue, impostor=purple. Whatever color the black slabs turn identifies the broken material (stays black ⇒ crown proxy or the classic-material fallback — then check the console for the `[trees] WebGPU tree node material failed` warning and fix the logged error instead of shipping the black fallback).
