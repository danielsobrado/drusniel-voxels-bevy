...
export interface WaterSunVisibilityAtlasSource {
...
export interface WaterSunVisibilityNodes {
...
export function buildWaterSunVisibilityNodes(
  worldXZ: TslNode,
  source: WaterSunVisibilityAtlasSource,
): WaterSunVisibilityNodes {
...
export function syncWaterSunVisibilityAtlas(source: WaterSunVisibilityAtlasSource): void {
  if (!shared) return;
  if (shared.texture !== source.texture) {
    throw new Error("water sun-visibility atlas texture identity changed");
  }
...
function getOrCreateSharedState(source: WaterSunVisibilityAtlasSource): SharedUniformState {
  if (!shared) {
    shared = {
  ...
  syncWaterSunVisibilityAtlas(source);
  return shared;
}
