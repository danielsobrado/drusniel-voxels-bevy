...
const MAX_SEARCH_CELLS_PER_AXIS = 512;
...
export function findContinentRiverCrossingRoute(
  ...
  const requestedSearchSpacingM = Math.max(1, finiteOption(options.searchSpacingM, DEFAULT_SEARCH_SPACING_M));
  const searchSpacingM = Math.max(
    requestedSearchSpacingM,
    searchRadiusM * 2 / MAX_SEARCH_CELLS_PER_AXIS,
  );
