...
describe("water foam runtime diagnostics", () => {
  it("reports the HQ material tier and fixed model contract", () => {
    const result = getWaterFoamRuntimeDiagnostics(params("waterQuality=high&waterPerf=0"));
  ...
  it("reports the forced performance tier", () => {
    const result = getWaterFoamRuntimeDiagnostics(params("waterQuality=high&waterPerf=1"));
    expect(result.qualityTier).toBe("low");
  });

  it("includes finite sun-atlas diagnostics", () => {
    const result = getWaterFoamRuntimeDiagnostics(params(""));
