...
function flowAngle(sample: DressingEnvironmentSample): number | null {
  return flowSpeed(sample) > 1e-6 ? Math.atan2(sample.flow[1], sample.flow[0]) : null;
}
...
export function evaluateHydrologyAffinity(
  ...
  if (classId === "river_cobbles") {
    ...
    return { accepted: shore >= -2 && shore <= 4 && flowSpeed(sample) >= 0.15 && compatibleBed, orientationRad: null };
  }
