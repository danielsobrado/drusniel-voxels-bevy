...
type RiverStatsController = WaterController & { getRiverStats?: () => WaterRiverDebugStats };

interface ColorBinding { value: string }
...
function addColorControl(
  ...
  folder.addColor(binding, "value").name(label).onChange((value: string) => onChange(fromHexColor(value)));
}

function addDeepWaterLookFolder(gui: GUI, visual: WaterVisual, rebuild: () => void): void {
  const folder = gui.addFolder("water / deep water look");
  addColorControl(folder, "deep color", visual.deepColor, (next) => { visual.deepColor = next; rebuild(); });
  addColorControl(folder, "shallow teal", visual.shallowColor, (next) => { visual.shallowColor = next; rebuild(); });
  addColorControl(folder, "foam color", visual.foamColor, (next) => { visual.foamColor = next; rebuild(); });
...
function addWaterRefractionFolder(
  ...
  const actions = { showRefraction: () => setDebugMode("refraction"), showFinal: () => setDebugMode("final") };
...
export function createWaterGui(gui: GUI, deps: WaterGuiDeps): void {
  ...
  const rebuildVisual = () => deps.waterController.updateVisual(visual);
  ...
  addWaterDebugFolder(gui, deps.waterDebugState, {
    onEnabled: (enabled) => { deps.setWaterEnabled(enabled); deps.waterController.setVisible(enabled); },
    onMode: setDebugMode,
    onClipmapTint: (enabled) => { deps.setWaterClipmapTint(enabled); deps.waterController.setClipmapTint(enabled); },
    onWireframe: (enabled) => { deps.setWaterWireframe(enabled); deps.waterController.setWireframe(enabled); },
    onDepthWrite: (on) => { deps.setWaterDepthWrite(on); visual.depthWrite = on; rebuildVisual(); },
