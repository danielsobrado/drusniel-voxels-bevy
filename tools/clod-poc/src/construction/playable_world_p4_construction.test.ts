...
function canonicalPieces(pieces: readonly PlacedConstructionPiece[]): unknown[] {
...
describe("playable-world P4 construction hardening", () => {
  ...
  it("round-trips thirty pieces by semantic state", () => {
    ...
    const expected = canonicalPieces(source.store.pieces);
    ...
    expect(canonicalPieces(restored.store.pieces)).toEqual(expected);
