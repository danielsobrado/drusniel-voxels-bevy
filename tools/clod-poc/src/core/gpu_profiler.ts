/**
 * Per-pass GPU timing attribution (TP-1).
 *
 ...
 * `renderer.info.render.timestamp`, which under-reports badly (~0.02 ms even
 * at 50 FPS) once the pool wraps. This profiler instead:
 ...
 *      frame's per-uid durations by label into a `gpuPasses` record,
 *   3. prunes the pool's uid→duration map (three never clears it — it grows
 *      per resolve batch) and our label map alongside.
 *
 ...
 * pool overflows once at boot and recovers at the first steady-state resolve.
 * Steady state stays well under the limit when timestamps resolve every frame.
 */
