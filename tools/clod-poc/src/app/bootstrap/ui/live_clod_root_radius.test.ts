...
function phase0Config(clodRadiusM: number): Phase0Config {
  return {
    phase0: {
...
describe("resolveLiveClodRootRadius", () => {
  it("uses the phase0 CLOD radius when the URL has no explicit stream radius", () => {
    expect(resolveLiveClodRootRadius(new URLSearchParams(), phase0Config(2048), 96)).toBe(2048);
  });

  it("lets an explicit URL stream radius override the phase0 CLOD radius", () => {
    expect(resolveLiveClodRootRadius(new URLSearchParams("liveClodRootRadius=512"), phase0Config(2048), 96)).toBe(512);
  });
});
