import type { WaterSpellVfxConfig } from "./spell_config.js";
import { WATER_FRAGMENT_SHADER_SOURCE, WATER_VERTEX_SHADER_SOURCE } from "./water_shader_sources.js";
...
export class WaterSpellRenderer {
  private layer: HTMLElement | null = null;
  private canvas: HTMLCanvasElement | null = null;
  private gl: WebGLRenderingContext | null = null;
  private state: WaterProgramState | null = null;
  ...
  constructor(private readonly config: WaterSpellVfxConfig) {}
  ...
  play(durationMs: number): void {
    ...
    this.layer.dataset.spell = "water";
  ...
  private createProgramState(gl: WebGLRenderingContext): WaterProgramState | null {
    const vertex = this.compileShader(gl, gl.VERTEX_SHADER, WATER_VERTEX_SHADER_SOURCE);
    const fragment = this.compileShader(gl, gl.FRAGMENT_SHADER, WATER_FRAGMENT_SHADER_SOURCE);
    if (!vertex || !fragment) {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.warn("[spells] Water shader link failed.", gl.getProgramInfoLog(program));
    ...
    if (aPosition < 0 || !uResolution || !uOrigin || !uTarget || !uTime || !uProgress || !uScale) {
      console.warn("[spells] Water shader uniforms are incomplete.");
      gl.deleteProgram(program);
      gl.deleteBuffer(buffer);
      return null;
    }
  ...
  private compileShader(gl: WebGLRenderingContext, type: number, source: string): WebGLShader | null {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      console.warn("[spells] Water shader compile failed.", gl.getShaderInfoLog(shader));
