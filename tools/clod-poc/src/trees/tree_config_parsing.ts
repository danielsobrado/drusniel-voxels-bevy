...
function parseLod(root: Record<string, unknown>, fallback: TreeLodSettings): TreeLodSettings {
  return {
    ...
    impostorEndM: fallback.impostorEndM,
    canopyFadeStartM: fallback.canopyFadeStartM,
    canopyFadeEndM: fallback.canopyFadeEndM,
...
function parseImpostors(root: Record<string, unknown>, fallback: TreeImpostorSettings): TreeImpostorSettings {
  return {
    ...
    bakeAgeLayers: boolFrom(root.bake_age_layers, fallback.bakeAgeLayers),
    ...
    atlasPaddingPx: clampedIntFrom(root.atlas_padding_px, fallback.atlasPaddingPx, 0, 16),
