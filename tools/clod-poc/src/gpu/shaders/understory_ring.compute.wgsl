...
fn understory_hash(cell: vec2<f32>, salt: u32) -> f32 {
  let seed_f = f32(params.settings_u.z);
  let salt_f = f32(salt);
  return fract(sin(dot(cell + vec2<f32>(seed_f + salt_f, seed_f * 0.37 + salt_f * 1.17), vec2<f32>(41.3, 289.1))) * 43758.5453);
}
...
fn river_understory_band(hydro: vec4<f32>) -> vec4<f32> {
  let enabled = hydro.w;
  let wet_mask = hydro.y;
  if (enabled < 0.5 || wet_mask <= 0.001) { return vec4<f32>(0.0, 0.0, 0.0, 1.0); }
...
fn understory_world_cell(slot_x: u32, slot_z: u32, grid: u32, cell_size: f32, camera_xz: vec2<f32>) -> vec2<f32> {
  ...
  return vec2<f32>(round((cam_cell.x - sx) / f32(safe_grid)) * f32(safe_grid) + sx, round((cam_cell.y - sz) / f32(safe_grid)) * f32(safe_grid) + sz);
}
...
struct EcologySample {
  forest_influence: f32, forest_edge: f32, shade: f32, moisture: f32,
  clearing: f32, density: f32, deadfall: f32,
};
fn sample_understory_ecology(x: f32, z: f32, height: f32, normal_y: f32, ground_weight: f32) -> EcologySample {
  ...
  if (ecology_enabled < 0.5) {
    result.forest_influence = 0.5; result.forest_edge = 0.5; result.shade = 0.5;
    result.moisture = 0.5; result.clearing = 0.5; result.density = clamp(ground_weight, 0.0, 1.0); result.deadfall = 0.25;
    return result;
  }
