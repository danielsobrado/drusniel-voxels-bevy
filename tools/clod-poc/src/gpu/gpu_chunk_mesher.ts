...
import { Y_CELLS, computeMeshDims, packMeshParams, packFieldParams, packDigEdits, assembleChunkMesh, DIG_EDIT_BYTES, FIELD_PARAM_WORDS, MESH_PARAM_WORDS } from "./gpu_mesh_buffers.js";
...
export interface ChunkMesh { positions: Float32Array; normals: Float32Array; materials: Float32Array; materialWeights?: Float32Array; materialWeightStride?: number; indices: Uint32Array; }
export interface GpuChunkMesherCreateResult { mesher: GpuChunkMesher | null; unavailable: WebGpuUnavailable | null; }
...
export class GpuChunkMesher {
  ...
  private constructor(private readonly device: GPUDevice, private readonly layout: GPUBindGroupLayout, private readonly vertexPipeline: GPUComputePipeline, private readonly quadPipeline: GPUComputePipeline, private readonly S: number, initialEditCapacity: number) {
    ...
    const mk = (label: string, size: number, usage: number) => device.createBuffer({ label: `gpu mesher ${label}`, size, usage });
  ...
  private writeView(buffer: GPUBuffer, data: Int32Array | Uint32Array): void { this.device.queue.writeBuffer(buffer, 0, data.buffer as ArrayBuffer, data.byteOffset, data.byteLength); }

  private makeBindGroup(): GPUBindGroup {
    return this.device.createBindGroup({ label: "gpu mesher bind group", layout: this.layout, entries: [
      { binding: 0, resource: { buffer: this.digEdits } }, { binding: 1, resource: { buffer: this.fieldParams } }, { binding: 2, resource: { buffer: this.meshParams } },
      { binding: 3, resource: { buffer: this.outPositions } }, { binding: 4, resource: { buffer: this.outNormals } }, { binding: 5, resource: { buffer: this.outMaterials } },
      { binding: 6, resource: { buffer: this.cellIndex } }, { binding: 7, resource: { buffer: this.outIndices } }, { binding: 8, resource: { buffer: this.indexCount } }, { binding: 9, resource: { buffer: this.vertexCount } },
    ] });
  }

  static async create(chunkSize: number, opts: { sharedDevice?: GPUDevice; initialEditCapacity?: number } = {}): Promise<GpuChunkMesherCreateResult> {
    let device = opts.sharedDevice ?? null;
    if (!device) {
      try { const result = await requestWebGpuDevice(); if (!result.ok) return { mesher: null, unavailable: result }; device = result.device; }
      catch (error) { return { mesher: null, unavailable: gpuMesherUnavailable(error) }; }
    }
    try {
      ...
      const layout = device.createBindGroupLayout({ label: "gpu mesher layout", entries: [
      ...
      return { mesher: new GpuChunkMesher(device, layout, vertexPipeline, quadPipeline, chunkSize, opts.initialEditCapacity ?? 32), unavailable: null };
    } catch (error) { return { mesher: null, unavailable: gpuMesherUnavailable(error) }; }
  }
  ...
  private async meshChunkInner(cx: number, cz: number, world: WorldBounds, edits: readonly ResolvedDigEdit[]): Promise<ChunkMesh> {
    if (edits.length > this.editCapacity) {
      ...
      this.digEdits = this.device.createBuffer({ label: "gpu mesher digEdits", size: this.editCapacity * DIG_EDIT_BYTES, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST });
      this.bindGroup = this.makeBindGroup();
    }
    ...
    vpass.setPipeline(this.vertexPipeline); vpass.setBindGroup(0, this.bindGroup); vpass.dispatchWorkgroups(Math.ceil(this.slotCount / WORKGROUP_SIZE)); vpass.end();
    const qpass = enc.beginComputePass();
    qpass.setPipeline(this.quadPipeline); qpass.setBindGroup(0, this.bindGroup); qpass.dispatchWorkgroups(Math.ceil(this.edgeCount / WORKGROUP_SIZE)); qpass.end();
    ...
    try { await this.countReadback.mapAsync(GPUMapMode.READ); countMapped = true; counts = new Uint32Array(this.countReadback.getMappedRange().slice(0)); }
    finally { if (countMapped) this.countReadback.unmap(); }
    ...
    try {
      ...
      return assembleChunkMesh(new Float32Array(posRB.getMappedRange().slice(0)), new Float32Array(nrmRB.getMappedRange().slice(0)), new Float32Array(matRB.getMappedRange().slice(0)), new Uint32Array(idxRB.getMappedRange().slice(0)), vc, ic);
    } finally {
      if (posMapped) posRB.unmap(); if (nrmMapped) nrmRB.unmap(); if (matMapped) matRB.unmap(); if (idxMapped) idxRB.unmap();
      posRB.destroy(); nrmRB.destroy(); matRB.destroy(); idxRB.destroy();
    }
  }

  destroy(): void {
    this.digEdits.destroy(); this.fieldParams.destroy(); this.meshParams.destroy(); this.outPositions.destroy(); this.outNormals.destroy(); this.outMaterials.destroy(); this.cellIndex.destroy(); this.outIndices.destroy(); this.indexCount.destroy(); this.vertexCount.destroy(); this.countReadback.destroy();
  }
}
