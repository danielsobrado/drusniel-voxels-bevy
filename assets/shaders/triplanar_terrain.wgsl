...
#ifdef TERRAIN_GPU_BIOME_SPLAT
#import "shaders/world_source/biome_splat.wgsl"::{
    biome_splat_resolve_triplanar_weights,
}
#endif
...
const WET_SAND_HEIGHT: f32 = 5.0;
const WET_SAND_DARKEN: f32 = 0.85;
const WET_SAND_MAX_STRENGTH: f32 = 0.55;
const WET_ROUGHNESS: f32 = 0.25;
...
fn compute_uv(world_coord: vec2<f32>) -> vec2<f32> { return world_coord / uniforms.tex_scale; }

fn triplanar_weights(world_normal: vec3<f32>) -> vec3<f32> {
    let normal_abs = abs(world_normal);
    var weights = vec3<f32>(pow(normal_abs.x, uniforms.blend_sharpness), pow(normal_abs.y, uniforms.blend_sharpness), pow(normal_abs.z, uniforms.blend_sharpness));
    return weights / max(weights.x + weights.y + weights.z, 0.001);
}

fn unpack_normal(sampled: vec3<f32>) -> vec3<f32> { return normalize(sampled * 2.0 - 1.0); }

fn reorient_normal(tn: vec3<f32>, wn: vec3<f32>, axis: i32) -> vec3<f32> {
    var n = normalize(vec3(tn.xy * uniforms.normal_intensity, tn.z));
...
fn parallax_offset(uv: vec2<f32>, view_dir: vec3<f32>) -> vec2<f32> {
    ...
    return uv - view_dir.xy * (height * uniforms.parallax_scale);
}

fn sample_albedo_single(uv: vec2<f32>, mat: i32) -> vec4<f32> {
    if (mat == 0) { return textureSample(grass_albedo, tex_sampler, uv); }
    if (mat == 1) { return textureSample(rock_albedo, tex_sampler, uv); }
    if (mat == 2) { return textureSample(sand_albedo, tex_sampler, uv); }
...
fn sample_albedo_tp(uv_yz: vec2<f32>, uv_xz: vec2<f32>, uv_xy: vec2<f32>, w: vec3<f32>, mat: i32, view_dir: vec3<f32>) -> vec4<f32> {
    ...
    if (mat == 1) { cy = parallax_offset(uv_yz, view_dir); cz = parallax_offset(uv_xz, view_dir); cx = parallax_offset(uv_xy, view_dir); }
#endif
    if (mat == 0) { return textureSample(grass_albedo, tex_sampler, cy) * w.x + textureSample(grass_albedo, tex_sampler, cz) * w.y + textureSample(grass_albedo, tex_sampler, cx) * w.z; }
    if (mat == 1) { return textureSample(rock_albedo, tex_sampler, cy) * w.x + textureSample(rock_albedo, tex_sampler, cz) * w.y + textureSample(rock_albedo, tex_sampler, cx) * w.z; }
    if (mat == 2) { return textureSample(sand_albedo, tex_sampler, cy) * w.x + textureSample(sand_albedo, tex_sampler, cz) * w.y + textureSample(sand_albedo, tex_sampler, cx) * w.z; }
    return textureSample(dirt_albedo, tex_sampler, cy) * w.x + textureSample(dirt_albedo, tex_sampler, cz) * w.y + textureSample(dirt_albedo, tex_sampler, cx) * w.z;
}

#ifdef TERRAIN_HEX_TILING
const HEX_BLEND_BAND_FRAC: f32 = 0.15;
fn hex_albedo_weight(frag_dist: f32) -> f32 {
    if (hex_tiling.enabled == 0u) { return 0.0; }
...
fn hex_normal_weight(frag_dist: f32) -> f32 {
    if (hex_tiling.enabled == 0u || hex_tiling.normal_enabled == 0u) { return 0.0; }
...
fn sample_hex_albedo_plane(world_coord: vec2<f32>, tex: texture_2d<f32>) -> vec4<f32> {
    ...
    return hex_color_sample(tex, tex_sampler, st, hex_tiling.rotation_strength, hex_tiling.color_border_contrast);
}
fn sample_hex_albedo_plane_mat(world_coord: vec2<f32>, mat: i32) -> vec4<f32> {
    if (mat == 0) { return sample_hex_albedo_plane(world_coord, grass_albedo); }
    if (mat == 1) { return sample_hex_albedo_plane(world_coord, rock_albedo); }
    if (mat == 2) { return sample_hex_albedo_plane(world_coord, sand_albedo); }
...
fn sample_albedo_tp_hextile(world_pos: vec3<f32>, w: vec3<f32>, mat: i32) -> vec4<f32> {
    return sample_hex_albedo_plane_mat(world_pos.yz, mat) * w.x + sample_hex_albedo_plane_mat(world_pos.xz, mat) * w.y + sample_hex_albedo_plane_mat(world_pos.xy, mat) * w.z;
}
fn sample_hex_normal_plane(world_coord: vec2<f32>, tex: texture_2d<f32>) -> vec2<f32> {
    let st = hex_planar_coords(world_coord, uniforms.tex_scale);
    return hex_normal_derivative(tex, tex_sampler, st, hex_tiling.rotation_strength, hex_tiling.normal_border_contrast);
}
fn sample_hex_normal_plane_mat(world_coord: vec2<f32>, mat: i32) -> vec2<f32> {
    if (mat == 0) { return sample_hex_normal_plane(world_coord, grass_normal); }
    if (mat == 1) { return sample_hex_normal_plane(world_coord, rock_normal); }
    if (mat == 2) { return sample_hex_normal_plane(world_coord, sand_normal); }
    return sample_hex_normal_plane(world_coord, dirt_normal);
}
fn sample_normal_tp_hextile(world_pos: vec3<f32>, tp_weights: vec3<f32>, base_normal: vec3<f32>, mat: i32) -> vec3<f32> {
    ...
    let surf_grad = surfgrad_from_triplanar_projection(tp_weights, deriv_yz, deriv_xz, deriv_xy, base_normal);
...
fn sample_normal_tp(uv_yz: vec2<f32>, uv_xz: vec2<f32>, uv_xy: vec2<f32>, w: vec3<f32>, wn: vec3<f32>, mat: i32, view_dir: vec3<f32>) -> vec3<f32> {
    ...
    if (mat == 0) { nx = textureSample(grass_normal, tex_sampler, cy).rgb; ny = textureSample(grass_normal, tex_sampler, cz).rgb; nz = textureSample(grass_normal, tex_sampler, cx).rgb; }
    else if (mat == 1) { nx = textureSample(rock_normal, tex_sampler, cy).rgb; ny = textureSample(rock_normal, tex_sampler, cz).rgb; nz = textureSample(rock_normal, tex_sampler, cx).rgb; }
    else if (mat == 2) { nx = textureSample(sand_normal, tex_sampler, cy).rgb; ny = textureSample(sand_normal, tex_sampler, cz).rgb; nz = textureSample(sand_normal, tex_sampler, cx).rgb; }
    else { nx = textureSample(dirt_normal, tex_sampler, cy).rgb; ny = textureSample(dirt_normal, tex_sampler, cz).rgb; nz = textureSample(dirt_normal, tex_sampler, cx).rgb; }
...
fn blend_albedo_tp(uv_yz: vec2<f32>, uv_xz: vec2<f32>, uv_xy: vec2<f32>, world_pos: vec3<f32>, w: vec3<f32>, mat: i32, view_dir: vec3<f32>, hex_w: f32) -> vec4<f32> {
    if (hex_w >= 1.0) { return sample_albedo_tp_hextile(world_pos, w, mat); }
    let plain = sample_albedo_tp(uv_yz, uv_xz, uv_xy, w, mat, view_dir);
    if (hex_w <= 0.0) { return plain; }
...
fn blend_normal_tp(uv_yz: vec2<f32>, uv_xz: vec2<f32>, uv_xy: vec2<f32>, world_pos: vec3<f32>, w: vec3<f32>, wn: vec3<f32>, mat: i32, view_dir: vec3<f32>, hex_w: f32) -> vec3<f32> {
    if (hex_w >= 1.0) { return sample_normal_tp_hextile(world_pos, w, wn, mat); }
...
fn terrain_puddle_normal(base_normal: vec3<f32>, world_xz: vec2<f32>, puddle_mask: f32) -> vec3<f32> {
    if (uniforms.puddle_normal_strength <= 0.001 || puddle_mask <= 0.001) { return base_normal; }
    ...
    return normalize(base_normal + vec3<f32>(n0 - 0.5, 0.0, n1 - 0.5) * uniforms.puddle_normal_strength * puddle_mask);
}
...
fn terrain_debug_wire_color(section: u32, lod: u32) -> vec3<f32> {
    ...
    if (section == 0u) { return lod_color; }
    return mix(lod_color, terrain_debug_section_color(section), 0.72);
}
fn apply_terrain_debug_wireframe(base_color: vec4<f32>, uv_b: vec2<f32>) -> vec4<f32> {
    ...
    return vec4<f32>(mix(base_color.rgb, terrain_debug_wire_color(section, lod), line * 0.85), base_color.a);
}
...
fn sample_mesher_sdf(world_pos: vec3<f32>) -> f32 {
    ...
    if (any(uvw < vec3<f32>(0.0)) || any(uvw > vec3<f32>(1.0))) { return 1.0; }
    return textureSampleLevel(iso_band_volume, iso_band_sampler, uvw, 0.0).r;
}
fn apply_terrain_iso_band_overlay(base_color: vec4<f32>, world_pos: vec3<f32>, world_normal: vec3<f32>) -> vec4<f32> {
    if (iso_band.epsilon <= 0.0) { return base_color; }
    let sdf_here = sample_mesher_sdf(world_pos);
    let band = 1.0 - smoothstep(iso_band.epsilon, iso_band.epsilon * 2.5, abs(sdf_here));
    var out = base_color;
    if (band > 0.001) { out = vec4<f32>(mix(out.rgb, vec3<f32>(1.0, 0.15, 0.85), band * 0.85), out.a); }
    if (abs(sdf_here) > iso_band.mismatch_threshold) {
        let mismatch = smoothstep(iso_band.mismatch_threshold, iso_band.mismatch_threshold * 2.0, abs(sdf_here));
...
fn fragment(in: VertexOutput, @builtin(front_facing) is_front: bool) -> @location(0) vec4<f32> {
#ifdef TERRAIN_CLOD_DITHER
    if (clod_interleaved_gradient_noise(in.position.xy) > clamp(uniforms.clod_fade, 0.0, 1.0)) { discard; }
#endif
    ...
    let legacy_w_total = dot(mat_weights, vec4<f32>(1.0));
    let legacy_w = mat_weights / max(legacy_w_total, 0.001);
    let terrain_biome_id = u32(round(clamp(in.uv.y, 0.0, 6.0)));
#ifdef TERRAIN_GPU_BIOME_SPLAT
    let w = biome_splat_resolve_triplanar_weights(legacy_w, terrain_biome_id, world_pos.y, WATER_LEVEL, 1.0 - abs(world_normal.y), true);
#else
    let w = legacy_w;
#endif
    ...
    if (w_max > 0.95) { proxy_albedo = sample_albedo_single(proxy_uv, mat_idx); }
    else {
    ...
    return vec4<f32>(mix(proxy_albedo.rgb * mix(0.72, 0.9, height_tint), fog_color, fog_mix), 1.0);
#endif
#ifdef TERRAIN_ATLAS_ONLY_DEBUG
    pbr_input.material.base_color = vec4<f32>(w.x + 0.20 * w.w, w.z + 0.35 * w.x, w.y + 0.15 * w.z, 1.0) * uniforms.base_color;
...
#else
    let skip_normals = frag_dist > 120.0;
#endif
#ifdef TERRAIN_SINGLE_PROJECTION_FAR
    let far_uv = compute_uv(world_pos.xz);
    if (w_max > 0.95) { albedo = sample_albedo_single(far_uv, mat_idx); }
    else {
...
#endif
        if (skip_normals) { final_normal = world_normal; }
        else {
#ifdef TERRAIN_HEX_TILING
    ...
    var roughness = w.x * GRASS_ROUGHNESS + w.y * ROCK_ROUGHNESS + w.z * SAND_ROUGHNESS + w.w * DIRT_ROUGHNESS;
    ...
    var final_albedo = mix(albedo, albedo * vec4<f32>(WET_SAND_DARKEN, WET_SAND_DARKEN, WET_SAND_DARKEN, 1.0), wet_strength);
    ...
    if (uniforms.rain_factor > 0.001 || uniforms.wetness > 0.001 || uniforms.snow_factor > 0.001) {
        ...
        final_albedo = mix(final_albedo, final_albedo * vec4<f32>(0.88, 0.91, 0.94, 1.0), rain_wet_strength * 0.55);
        ...
        if (rain_wet_strength > 0.001 && uniforms.puddle_strength > 0.001) {
            ...
            if (!skip_normals) { blended_n = terrain_puddle_normal(blended_n, world_pos.xz, puddle_mask); }
        }
        snow_mask = terrain_snow_mask(world_normal);
        final_albedo = mix(final_albedo, vec4<f32>(max(final_albedo.rgb, vec3<f32>(0.88, 0.90, 0.94)), final_albedo.a), snow_mask);
        roughness = mix(roughness, 0.78, snow_mask);
    }
    var final_ao = ao_factor;
    if (uniforms.procedural_support_maps_enabled > 0.5) {
        let procedural_material = sample_procedural_terrain_material(world_pos, world_normal, w, frag_dist, final_albedo.rgb, blended_n, roughness, ao_factor, uniforms.procedural_snow_mask, uniforms.procedural_wet_mask, uniforms.procedural_slope_masks, uniforms.procedural_tint_strengths, uniforms.procedural_material_roughness, uniforms.procedural_moss_tint, uniforms.procedural_gravel_tint, uniforms.procedural_wet_tint, uniforms.procedural_snow_tint, uniforms.procedural_material_params);
    ...
    if ((uniforms.weather_flags & WEATHER_DEBUG_PUDDLE) != 0u) { final_albedo = vec4<f32>(vec3<f32>(puddle_mask), 1.0); roughness = 0.9; }
    else if ((uniforms.weather_flags & WEATHER_DEBUG_WETNESS) != 0u) { final_albedo = vec4<f32>(vec3<f32>(rain_wet_strength), 1.0); roughness = 0.9; }
    else if ((uniforms.weather_flags & WEATHER_DEBUG_SNOW) != 0u) { final_albedo = vec4<f32>(vec3<f32>(snow_mask), 1.0); roughness = 0.9; }
    ...
    if (DEBUG_FORCE_ALBEDO) { pbr_input.material.base_color = DEBUG_ALBEDO_COLOR; pbr_input.material.flags |= pbr_types::STANDARD_MATERIAL_FLAGS_UNLIT_BIT; }
    var color: vec4<f32>;
    if ((pbr_input.material.flags & pbr_types::STANDARD_MATERIAL_FLAGS_UNLIT_BIT) == 0u) { color = pbr_functions::apply_pbr_lighting(pbr_input); }
    else { color = pbr_input.material.base_color; }
    if (world_pos.y < WATER_LEVEL) {
        ...
        let caustic = water_caustics::calculate_caustics(world_pos, WATER_LEVEL, globals.time, 0.85, 1.2) * shoreline_caustic_falloff * caustic_surface_mask;
